    ...
    Element.implement({
        set: function (r, o) {
            ...
            case "object":
                ...
                break;
            case "string":
                var n = Element.Properties.get(r);
    ...
    Element.implement({
        ...
        getOffsets: function () {
            if (this.getBoundingClientRect) {
                ...
                return {
                    x: l.left.toInt() + m.x - k.x + ((j) ? 0 : s.x) - o.clientLeft,
                    y: l.top.toInt() + m.y - k.y + ((j) ? 0 : s.y) - o.clientTop
                }
            }
            var n = this,
                q = {
        ...
        getPosition: function (m) {
            ...
            var l = (m && (m = document.id(m))) ? m.getPosition() : {
                x: 0,
                y: 0
            };
            return {
    ...
    Element.implement({
        position: function (h) {
            ...
            case "left":
                ...
                break;
            case "right":
                l.x = j + t + g.offsetWidth;
                break;
            default:
                l.x = j + ((g == document.body ? m.x : g.offsetWidth) / 2) + t;
                break
            }
            switch (h.position.y) {
            case "top":
                ...
                break;
            case "bottom":
                l.y = n + r + g.offsetHeight;
                break;
            default:
                l.y = n + ((g == document.body ? m.y : g.offsetHeight) / 2) + r;
                break
            }
            if (h.edge) {
                var b = {};
                ...
                case "left":
                    ...
                    break;
                case "right":
                    b.x = -q.x - q.computedRight - q.computedLeft;
                    break;
                default:
                    b.x = -(q.totalWidth / 2);
                    break
                ...
                case "top":
                    ...
                    break;
                case "bottom":
                    b.y = -q.y - q.computedTop - q.computedBottom;
                    break;
                default:
                    b.y = -(q.totalHeight / 2);
                    break
...
Drag.Move = new Class({
    ...
    calculateLimit: function () {
        ...
        var f = this.element.offsetWidth + c.left + c.right,
            q = this.element.offsetHeight + c.top + c.bottom,
            j = 0,
            l = 0,
            o = h.right - g.right - f,
            a = h.bottom - g.bottom - q;
...
var Autocompleter = new Class({
    ...
    setSelection: function (g) {
        var h = this.selected.inputValue,
            j = h;
        var a = this.queryValue.length,
            c = h.length;
    ...
    showChoices: function () {
        ...
        if (this.fix) {
            var f = this.element.getCoordinates(this.relative),
        ...
        if (this.options.visibleChoices) {
            var j = document.getScroll(),
                m = document.getSize(),
                h = this.choices.getCoordinates();
            if (h.right > j.x + m.x) {
                j.x = h.right - m.x
            }
            if (h.bottom > j.y + m.y) {
    ...
    update: function (b) {
        ...
        if (!a || (a == "array" && !b.length) || (a == "hash" && !b.getLength())) {
            ...
            b.each(this.options.injectChoice ||
            function (d) {
    ...
    choiceOver: function (c, d) {
        ...
        if (this.overflown) {
            var g = this.selected.getCoordinates(this.choices),
                f = this.options.overflowMargin,
                h = this.choices.scrollTop,
...
Travian.Dialog = new Class({
    ...
    render: function () {
        ...
        this.form = this.wrapper.down("form").addEvent("submit", function (b) {
            ...
            if (Browser.Engine.trident && (a.options.submitMethod || a.options.submitUrl)) {
                this.buttonOk.addClass("disabled").disabled = true
            } else {
    ...
    update: function () {
        ...
        var a = {
            left: b.x + g.x + f.x / 2 - c.width / 2,
            top: b.y + g.y + f.y / 2 - c.height / 2
        };
        if (a.left < 0) {
            a.left = 5
        }
...
Travian.Game = {
    ...
    showEditVillageDialog: function (f, b, d, c) {
        var a = b + ' <input type="text" id="villageNameInput" name="villageName" value="" />';
        a.dialog({
            title: f,
            ...
            onOkay: function (g, h) {
                Travian.ajax({
                    ...
                    onSuccess: function (j) {
                        $("villageNameField").innerHTML = j.name
                    }
                })
            },
            onOpen: function (g, h) {
...
Travian.Game.RaidList = {
    ...
    editSlot: function (b, c) {
        ...
        window.location.href = "build.php?gid=16&tt=99&action=showSlot&eid=" + c + "&sort=" + a + "&direction=" + f
    },
...
Travian.Game.Hero.Editor = new Class({
    ...
    sendAction: function (a, c) {
        ...
        Travian.ajax({
            ...
            onSuccess: function (d) {
                $$(".hero_head .image").set("html", d.html);
                b.options.attributes = d.attributes;
                b.storeAttributesInInput(d.attributes);
...
Travian.Game.Hero.Inventory = new Class({
    ...
    createDivs: function () {
        ...
        this.items.each(function (h, d) {
            ...
            var f = (new Element("div", {
                id: "item_" + h.id,
                "class": "item item_" + h.typeId + " " + (h.isUseable ? "" : "disabled"),
...
Travian.Game.Map.Base = new Class({
    ...
    initialize: function (a, c) {
        ...
        $each(this.options, function (f, d) {
            b[d] = f
        });
        if (this.id == null) {
...
Travian.Game.Map.Container = (function () {
    ...
    return new Class({
        ...
        initialize: function (b) {
            ...
            var f = $extend(this.containerRender.getDimensions({
                computeSize: true
        ...
        renderBlocks: function () {
            ...
            for (var j = 0, g = 0; j < b; j++) {
                for (var k = 0; k < c; k++) {
                    h = Travian.Game.Map.Layer.Block.getCorrectPosition({
...
Travian.Game.Map.Updater = (function () {
    ...
    var d = function (g) {
            if (g.requestCountImages >= g.maxRequestCount) {
                return
            }
            if (g.objects.images.getLength() == 0) {
                return
            }
    ...
    var d = function (n) {
            ...
            for (var g = 0, h = null, k = null; g < o; g += m) {
                h = (new Element("div", {
                    ...
                    styles: {
                        position: "absolute",
                        left: g * n.mapContainer.blockSize.width / n.transition.elementsPerBlock.x,
                        top: 0,
            ...
            for (var g = 0, h = null, k = null; g < l; g += j) {
                h = (new Element("div", {
                    ...
                    styles: {
                        ...
                        width: n.containerSize.width,
                        height: j * n.mapContainer.blockSize.height / n.transition.elementsPerBlock.y
                    }
...
Travian.Game.Map.MiniMap = (function () {
    ...
    if (Browser.Engine.presto) {
        c = function (f) {
            ...
            f.elements.background.setStyles({
                bottom: 0
            });
            (function () {
                f.elements.background.setStyles({
    ...
    return new Class({
        ...
        render: function () {
            ...
            this.container = $(this.container).setStyles({
                overflow: "hidden"
            }).disableSelection().addEvent("click", function (j) {
...
Travian.Game.Map.MapMark.Layer.Mark = new Class({
    ...
    addData: function (a) {
        var b = new Travian.Game.Map.MapMark.Layer.Data.Mark($merge(a, this.optionsData, {
...
Travian.Game.Map.MapMark.Layer.Flag = new Class({
    ...
    addData: function (b) {
        var a = new Travian.Game.Map.MapMark.Layer.Data.Flag($merge(b, this.optionsData, {
            editable: this.editable
        }), this);
        this.datas[a.id] = a;
        return this
                ...
                this.elements.select.addEvent("click", function (d) {
                    c.parentContainer.showDialog({
                        ...
                        onOpen: function (g, h, f) {
                            h.select(".coord").each(function (j) {
                                j.hide()
                            });
